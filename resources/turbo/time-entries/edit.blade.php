<turbo-frame id="time-entry-{{ $timeEntry->id }}">
  <div class="card-body space-y-6">
    ...
    <!-- Form -->
    <form action="{{ route('turbo.time-entries.update', $timeEntry) }}" method="POST" class="space-y-6" data-turbo-frame="time-entry-{{ $timeEntry->id }}">
      @csrf
      @method('PUT')
      @if($is_recent)
      ...
      <div>
        ...
        <x-form.money-input
          :value="old('hourly_rate_amount', $timeEntry->hourlyRate?->rate?->toDecimal())"
          :currency="old('hourly_rate_currency', $timeEntry->hourlyRate?->rate?->currency)"
          :project="$timeEntry->project"
          :client="$timeEntry->client"
