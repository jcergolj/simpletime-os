    ...
    <form action="{{ route('turbo.projects.update', $project) }}" method="POST" class="space-y-6" data-turbo-frame="project-{{ $project->id }}">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div class="form-control">
          ...
          <x-form.money-input
            :value="old('hourly_rate_amount', $project->hourlyRate?->rate?->toDecimal() ?? $project->client?->hourlyRate?->rate?->toDecimal() ?? auth()->user()?->hourlyRate?->rate?->toDecimal())"
            :currency="old('hourly_rate_currency', $project->hourlyRate?->rate?->currency ?? $project->client?->hourlyRate?->rate?->currency ?? auth()->user()?->hourlyRate?->rate?->currency)"
