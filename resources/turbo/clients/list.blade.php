<div class="card mx-4 sm:mx-0" style="padding: 0;">
    <div style="padding: 28px 32px; border-bottom: 1px solid var(--border);">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            <div>
                <h3 style="font-size: 28px; font-weight: 600; color: var(--text); margin-bottom: 4px;">{{ __('Your Clients') }}</h3>
                <p style="font-size: 14px; color: var(--text-secondary);">{{ $clients->total() }} {{ Str::plural(__('client'), $clients->total()) }} {{ __('total') }}</p>
            </div>
            @if($clients->hasPages())
        ...
        <div style="padding: 28px 32px; border-bottom: 1px solid var(--border);" class="last:border-b-0">
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                ...
                <div class="flex items-center gap-2">
                    <a href="{{ route('turbo.clients.edit', $client) }}" style="padding: 8px 16px; background: var(--accent); color: white; border-radius: 8px; font-size: 14px; font-weight: 500; line-height: 1; display: inline-flex; align-items: center; gap: 6px; text-decoration: none; border: none;">
    ...
    @empty
        <div style="padding: 64px 32px; text-align: center;">
            ...
            <h3 style="font-size: 18px; font-weight: 600; margin-bottom: 8px; color: var(--text);">{{ __('No clients yet') }}</h3>
            <p style="font-size: 14px; color: var(--text-secondary); margin-bottom: 20px;">{{ __('Get started by adding your first client above.') }}</p>
            <a href="{{ route('turbo.clients.create') }}" style="padding: 12px 24px; background: var(--accent); color: white; border-radius: 8px; font-size: 15px; font-weight: 500; line-height: 1; display: inline-flex; align-items: center; gap: 6px; text-decoration: none; border: none;">
                <svg style="width: 18px; height: 18px; flex-shrink: 0;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
                </svg>
                <span style="line-height: 1;">{{ __('Add Your First Client') }}</span>
            </a>
        </div>
    @endforelse
