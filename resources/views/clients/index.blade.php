<x-layouts.app :title="__('Clients')">
    <div class="space-y-8" data-controller="inline-edit">
        ...
        <div class="px-4 sm:px-0">
            <h1 style="font-size: 48px; font-weight: 700; margin-bottom: 8px; letter-spacing: -0.5px; color: var(--text);">{{ __('Clients') }}</h1>
            <p style="font-size: 17px; color: var(--text-secondary); font-weight: 400;">{{ __('Manage your clients and their hourly rates') }}</p>
        </div>

        <!-- Search Filter -->
        <div class="card mx-4 sm:mx-0" style="padding: 32px 28px;">
            <h3 style="font-size: 24px; font-weight: 600; margin-bottom: 24px; color: var(--text);">{{ __('Search Clients') }}</h3>
        ...
        <div class="card mx-4 sm:mx-0" style="padding: 32px 28px;">
            <turbo-frame id="client-create-form">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
                    <div>
                        <h2 style="font-size: 24px; font-weight: 600; margin-bottom: 8px; color: var(--text);">{{ __('Add New Client') }}</h2>
                        <p style="font-size: 15px; color: var(--text-secondary);">{{ __('Create a new client profile with billing information') }}</p>
                    </div>
                    <a href="{{ route('turbo.clients.create') }}" class="btn-primary sm:w-auto" style="display: inline-flex; align-items: center; justify-content: center; gap: 6px; line-height: 1; text-decoration: none;">
                        <svg style="width: 18px; height: 18px; flex-shrink: 0;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
